...
sig
  (** Totally ordered set that can be embedded into the real line. The
      function "compare" must be compatible with respect to the
	...
	then
	  ...
	  then
	    match !kept with
	      ...
	      | Some (Opn z as b) ->
		if loading then b::
		  if (try clear () ; x <= rvb (List.hd ar2') with | Failure _ -> true)
		  then
		    if circle_mode && ar2'=[]
		    then [Opn y]
		  ...
		  if (try x <= rvb (List.hd ar2') with | Failure _ -> true)
		  then
		    if circle_mode && ar2'=[]
		    then [Opn y]
		    else (load (Opn y); past_extension false x1 (true,ar2'))
...
module HalfLine(B:Bound):OdaHalfLine
  with type value = B.t
...
module Circle(B:Bound):OdaCircle
  with type value = B.t
  and type bound = BooleanAlgebra(B).bound
  and type t = BooleanAlgebra(B).t
	=
struct
  ...
  let closure_contains_zero a =
	...
	let boundary_contains_zero a =
		(try List.hd a = Opn zero with Failure _ -> false)
    || ((try rvb (List.hd a) <> zero with Failure _ -> true) && parity false a)
    || ((try List.hd a = Cls zero with Failure _ -> false) && parity true a)

	(*Added on Wednesday, the 23th of November 2011*)
	let boundary_does_not_contain_zero a =
		((try List.hd a = Cls zero with Failure _ -> false) && parity false a)
    || ((try rvb (List.hd a) <> zero with Failure _ -> true) && parity true a)
