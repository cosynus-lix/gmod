...
let parse_display d =
  ...
  let handle = function
    ...
    | ["resources"] ->
      header "Resources" (fun _ -> Semantics.resources)
    ...
    | ["equations"]
    | ["eq"] ->
      header "Process definitions" (fun _ sem -> Semantics.equations sem None)
    | ["equation-names"]
    | ["en"] ->
      header "Process names" (fun _ sem -> Semantics.equation_names sem.Semantics.equations)
    | ["the-equation"]
    | ["te"] ->
      header "Some process definitions" (fun _ -> Semantics.specify_equations)
    | ["critical-section"]
    | ["csec"] ->
    ...
    | ["runpro"]
    | ["rp"] ->
      header "Running processes" (fun _ -> Semantics.running_processes)
    | ["factorize"]
    | ["fac"] ->
