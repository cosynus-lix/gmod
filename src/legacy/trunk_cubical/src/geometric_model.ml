...
struct
  ...
  let forbidden ?(local=false) ?runpro ?akgs syntactical_semantics =
    ...
    (*
      let msg =
      match s with
      | "switch semaphore" -> if syntactical_semantics.Semantics.sem_switch_tbl = Mos.empty then "done\n" else ""
      | "quantitative semaphore" -> if syntactical_semantics.Semantics.sem_quantitative_tbl = Mos.empty then "done\n" else ""
      | "recursive mutex" -> if syntactical_semantics.Semantics.mtx_recursive_set = Sos.empty then "done\n" else ""
      | "switch mutex" -> if syntactical_semantics.Semantics.mtx_switch_set = Sos.empty then "done\n" else ""
      | "normal mutex" -> if syntactical_semantics.Semantics.mtx_normal_set = Sos.empty then "done\n" else ""
      | "monitor" -> if syntactical_semantics.Semantics.mntset = Sos.empty then "done\n" else ""
      | "queue" -> if syntactical_semantics.Semantics.filtbl = Mos.empty then "done\n" else ""
      | "stack" -> if syntactical_semantics.Semantics.piltbl = Mos.empty then "done\n" else ""
      | "synchronization" -> if syntactical_semantics.Semantics.syntbl = Mos.empty then "done\n" else ""
      | _ ->
  ...
  let local_deadlock_with_parameter ?runpro ?akgs n syntactical_semantics =
	...
	try
	  while !answer = (AC.empty ~d:dimension ()) do
	    ...
	    current := Algebra.SequencesOfIntegers.next_list !current (dimension-1)
	  done
	  ;
	  (akgs.local_deadlock_with_parameter <- Some !answer)
	with
	| Not_found (*Exit*) -> (akgs.local_deadlock_with_parameter <- Some !answer)
      )
      ;
