...
struct
  ...
  struct
    ...
    let rec of_expr var = function
      | Semantics.Ac n -> make (n, n)
      | Semantics.Va v -> var v
      | Semantics.Pl (e1, e2) -> add (of_expr var e1) (of_expr var e2)
      | _ -> assert false
...
module Environment = struct
  ...
  let transition e p =
    ...
    match p with
      | Semantics.D (Semantics.T_int,v) -> A.add_var e v
      | Semantics.T (v,expr) -> A.assign e v expr
      | _ -> e
