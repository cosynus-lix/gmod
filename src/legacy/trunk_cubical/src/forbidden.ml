...
let pile_starvation runpro lifo = (* Coming from starvation *)
  ...
  AC.complement (AC.locus_higher_than ~filtration:true ~g:(snd aux) 0 (fst aux))
...
let pile_starvation ?runpro syntactical_semantics =
  let runpro =
    match runpro with
      ...
      | Some x -> x
  in
  let lifo = Semantics.lifo syntactical_semantics in
  AC.compress
    (List.fold_left
       (fun accu (lifo,n) ->
...
let pile_overflow runpro lifo arity = (* Coming from overflow *)
  let aux = Buzy_matrix.ifo runpro lifo in
  AC.locus_higher_than ~filtration:true ~g:(snd aux) (arity+1) (fst aux)
...
let file_starvation runpro fifo = (* Coming from starvation *)
  ...
  AC.complement
     (AC.locus_higher_than ~filtration:true ~g:(snd aux) 0 (fst aux))
...
let file_starvation ?runpro syntactical_semantics =
  let runpro = match runpro with
    ...
    | Some x -> x
  in
  let fifo = Semantics.fifo syntactical_semantics in
  AC.compress
    (List.fold_left
       (fun accu (f,n) ->
...
let quantitative_semaphore runpro sem arity =
  AC.locus_higher_than (*AC.levelwise_product*) arity (Buzy_matrix.quantitative runpro sem arity)
...
let quantitative_semaphore ?runpro syntactical_semantics =
  let runpro = match runpro with
    ...
    | Some x -> x
  in
  let semq = Semantics.semaphore `Quantitative syntactical_semantics in
  AC.compress
    (List.fold_left
       (fun accu (s,n) ->
...
let recursive_mutex ?runpro syntactical_semantics =
  let runpro = match runpro with
    ...
    | Some x -> x
  in
  let mtx = Semantics.mutex `Recursive syntactical_semantics in
  AC.compress
    (List.fold_left
	(fun accu m ->
          Printf.printf "  - treating %s\n%!" m;
...
let switch_mutex ?runpro syntactical_semantics =
  let runpro = match runpro with
    ...
    | Some x -> x
  in
  let mtx = Semantics.mutex `Switch syntactical_semantics in
  AC.compress
    (List.fold_left
       (fun accu m ->
         Printf.printf "  - treating %s\n%!" m;
...
let normal_mutex ?runpro syntactical_semantics =
  let runpro = match runpro with
    ...
    | Some x -> x
  in
  let mtx = Semantics.mutex `Normal syntactical_semantics in
  AC.compress
    (List.fold_left
       (fun accu m ->
         Printf.printf "  - treating %s\n%!" m;
...
let switch_semaphore ?runpro syntactical_semantics =
  let runpro = match runpro with
    ...
    | Some x -> x
  in
  let sem = Semantics.semaphore `Switch syntactical_semantics in
  AC.compress
    (List.fold_left
       (fun accu (s,n) ->
...
let synchronization ?runpro syntactical_semantics =
  let runpro = match runpro with
    | None -> Interpreter.init_runpro syntactical_semantics
    | Some x -> x in
  let sync = Semantics.synchronization syntactical_semantics in
...
let monitor ?runpro syntactical_semantics =
  let runpro = match runpro with
    ...
    | Some x -> x
  in
  let mon = Semantics.monitor syntactical_semantics in
  List.fold_left
    (fun accu m ->
      Printf.printf "  - treating %s\n%!" m;
...
let conflict ?runpro ?(out=false) ?forbidden syntactical_semantics =
  ...
  let conflict_area =
    AC.join
	 ...
	 (AC.empty ~d:(Array.length runpro) ())
	 syntactical_semantics.Semantics.variables
      )
      (List.fold_left
	 (fun accu var ->
