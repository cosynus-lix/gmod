         ...
         (fun i ii ->
           ...
           | Call (_, Lval (Var v, NoOffset), args, loc) when v.vname = "pthread_mutex_lock" ->
             ...
             (
               ...
               | AddrOf (Var v, NoOffset) ->
                 [AbstractSyntax.P v.vname,pos loc]
             )
           | Call (_, Lval (Var v, NoOffset), args, loc) when v.vname = "pthread_mutex_unlock" ->
             ...
             (
               match m with
               | AddrOf (Var v, NoOffset) ->
                 [AbstractSyntax.V v.vname,pos loc]
             )
           | Call (_,_,_,loc) | Set (_,_,loc) -> [AbstractSyntax.Nop, pos loc]
         ) i)
  in
  ...
  let resources =
    List.map
      (fun (v, kind) ->
        ...
        | `Mutex mode -> AbstractSyntax.Mutex mode
        | `Barrier None -> failwith (Printf.sprintf "Barrier %s has unknown arity." name)
        | `Barrier (Some n) -> AbstractSyntax.Synchronization n
      ) !resources
  in
  let semantics =
    {
