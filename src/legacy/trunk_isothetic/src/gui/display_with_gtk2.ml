...
let abstract_interpretation = ref (fun (_:AbstractSyntax.instr list) -> ([]:string list))
...
class gui () =
  ...
  let (* nb_semantics *) _ = notebook#append_page ~tab_label:(GMisc.label ~text:"AbstractSyntax" ())#coerce semantics#semantics#coerce in
...
object (self)
  ...
  method on_abstract_testing =
    ...
    let max_coord = Array.init dim (fun k -> Array.length runpro.(k).AbstractSyntax.code + 1) in
    let n = ref (-1) in
    let tree =
      List.map
        (fun p ->
	    ...
	    (* Lift the path from geometry to the code. *)
            Path_extract.generic
              ~runpro
              (fun ?runpro k ip ->
                fst (get_some runpro).(k).AbstractSyntax.code.(ip.(k)-1),
                Array.to_list
                  (
                    Array.init dim
                      (fun k ->
                        let code = (get_some runpro).(k).AbstractSyntax.code in
			let ipk = ip.(k) in
