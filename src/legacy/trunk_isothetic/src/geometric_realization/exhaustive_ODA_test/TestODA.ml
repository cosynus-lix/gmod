...
struct
  module I = Interval.Make(HalfLine.Integer)
  module A = Arc.Make(HalfLine.Integer)
  ...
  module Sh = ODA.BooleanAlgebra(HalfLine.Integer)
  module HL = ODA.HalfLine(HalfLine.Integer)
  module Ci = ODA.Circle(HalfLine.Integer)
...
(*
    let () = 
      let a = [Sh.Cls 0 ; Sh.Pun 1; Sh.Pun 2] in
      let a = [Sh.Cls 1 ; Sh.Cls 2] in
      if Ci.boundary_contains_zero a
      then (
        Printf.printf "The boundary of %s contains zero\n" (Ci.string_of a)
      )
      else (
        Printf.printf "The boundary of %s does not contain zero\n" (Ci.string_of a)
      )
*)
    ...
    let rec to_area ?p a = match a with
      ...
      | [] -> (
            match p with
              ...
              | None -> []
              | _ -> failwith "test.ml: to_area: invalid form 4") in
    try
      if a <> []
      then AC.of_list (to_area a)
      else AC.empty ~d:1 ()
    with Failure _ -> failwith (Sh.string_of a)

(*
  let to_area a = 
    let output = to_area a in
    let () = Printf.printf "to_area %s = %s\n%!" (Sh.string_of a) (AC.string_of output) ; flush stdout in
    output
*)
...
*)
	    let a = Algebra.SequencesOfIntegers.next_list (List.map Sh.rvb a) upper_bound in
...
*)
		    (Globals.segment_initial_croistart next_level 0)
	      )
	)
...
"
  ...
  let () = 
    ...
    then test_compare [Sh.Opn 0] [Sh.Cls 0]
  ...
  let testing_union i j =
    ...
    if !verbose then Printf.printf "Start testing %i (%i) %i (%i).\n" i (List.length (test i)) j (List.length (test j)) ;
    List.iter (
      fun op1 -> (
        List.iter (
          fun op2 -> 
...
(* TODO *)
  ...
  let testing_belongs_to i =
    let counter = ref 0 in
    if !verbose then Printf.printf "Start testing %i (%i).\n" i (List.length (test i)) ;
    List.iter
      (
	fun pt1 ->
	  (
  ...
  let full_testing_union () =
    print_endline "Testing union begin";
    for i=0 to 15 do
      for j=0 to 15 do
        testing_union i j
      done
    done;
    print_endline "Testing union end"

  let full_testing_intersection () =
    print_endline "Testing intersection begin";
    for i=0 to 15 do
      for j=0 to 15 do
        testing_intersection i j
      done
    done;
    print_endline "Testing intersection end"

  let full_testing_difference () =
    print_endline "Testing difference begin";
    for i=0 to 15 do
      for j=0 to 15 do
        testing_difference i j
      done
    done;
    print_endline "Testing difference end"
  ...
  let full_testing_future_extension_unbounded_flag () = 
    print_endline "Testing future_extension_unbounded_flag (halfline mode) begin";
    for i=0 to 15 do
      for j=0 to 31 do
        testing_future_extension_unbounded_flag i j
      done
    done;
    print_endline "Testing future_extension_unbounded_flag (halfline mode) end"
  ...
  let full_testing_is_included () =
    print_endline "Testing is_included begin";
    for i=0 to 31 do
      for j=0 to 31 do
        testing_is_included i j
      done
    done;
    print_endline "Testing is_included end"

  let full_testing_belongs_to () =
    print_endline "Testing belongs_to begin";
    for i=0 to 15 do
      testing_belongs_to i
    done;
    print_endline "Testing belongs_to end"

  let full_testing_complement () =
    print_endline "Testing complement begin";
    for i=0 to 15 do
      testing_complement i
    done;
    print_endline "Testing complement end"

  let full_testing_interior () = 
    print_endline "Testing interior (halfline mode) begin";
    for i=0 to 15 do
      testing_interior i
    done;
    print_endline "Testing interior (halfline mode) end"

  let full_testing_closure () = 
    print_endline "Testing closure (halfline mode) begin";
    for i=0 to 15 do
      testing_closure i
    done;
    print_endline "Testing closure (halfline mode) end"

  let full_testing_boundary () =
    print_endline "Testing boundary (halfline mode) begin";
    for i=0 to 15 do
      testing_boundary i
    done;
    print_endline "Testing boundary (halfline mode) end"

  let full_testing_to_area () = 
    print_endline "Testing to_area (halfline mode) begin";
    for i=0 to 15 do
      testing_to_area i
    done;
    print_endline "Testing to_area (halfline mode) end"
...
(* to be tested *)
  ...
  let to_area a = 
      ...
      | Sh.Iso x::a ->
          let answer = to_area a in (
            match !last_bound with
              | Some Sh.Cls y -> (
                  if x <> Ci.zero 
                  then A.bounded true false y Ci.zero::(A.atom x)::answer
                  else A.bounded true true y Ci.zero::answer)
              | Some Sh.Pun y
              | Some Sh.Opn y -> (
                  if x <> Ci.zero 
                  then A.bounded false false y Ci.zero::A.atom x::answer
                  else A.bounded false true y Ci.zero::answer)
	...
	then [A.bounded true false x Ci.zero]
      ...
      | [Sh.Cls x;Sh.Pun y] ->
          if x<>Ci.zero
          then [A.terminal false y;A.bounded true false x y]
          else [A.coatom y]
      | Sh.Cls x::a -> 
              ...
              | Some Sh.Cls y -> (
                  ...
                  then
                    ...
                    A.dummy_union 
                        ...
                        (List.hd answer))::(List.tl answer))
              | Some Sh.Pun y
              | Some Sh.Opn y -> (
                  if x <> Ci.zero
                  then A.bounded false false y Ci.zero::answer
                  else (
                      A.dummy_union 
	...
	then [A.terminal false x] (*[A.bounded false false x Ci.zero]*)
      ...
      | [] -> [A.empty]
      (* dump *)
      | _ -> failwith "test.ml:Circle.to_area NIY" in
    List.fold_left 
  ...
  let to_area a = 
    let output = to_area a in
    let () = if false then Printf.printf "to_area %s = %s\n%!" (Sh.string_of a) (AT.string_of output) ; flush stdout in
    output
  ...
  let full_testing_string_of () =
    print_endline "Testing string_of (circle mode) begin";
    for i=0 to 15 do
      testing_string_of i
    done;
    print_endline "Testing string_of (circle mode) end"
  ...
  let full_testing_interior () =
    print_endline "Testing interior (circle mode) begin";
    for i=0 to 15 do
      testing_interior i
    done;
    print_endline "Testing interior (circle mode) end"

  let full_testing_closure () =
    print_endline "Testing closure (circle mode) begin";
    for i=0 to 15 do
      testing_closure i
    done;
    print_endline "Testing closure (circle mode) end"
...
(*
              let answer2 = future_extension (to_area op1) (to_area op2) in
*)
              let answer2 = AT.meet (to_area op2) (future_extension (to_area op1) (to_area op2)) in
              if (
                try 
                  not (
                    (AT.is_included (to_area answer1) answer2)
                    && (AT.is_included answer2 (to_area answer1)) )
                with _ -> true)
              then (
...
(*
			let answer2 = past_extension (to_area op1) (to_area op2) in
*)
			let answer2 = AT.meet (to_area op2) (past_extension (to_area op1) (to_area op2)) in
			if (not((AT.is_included (to_area answer1) answer2)&&(AT.is_included answer2 (to_area answer1))))
			then
			  (
