...
sig
  type regular_value
  type area
  val recursive_mutex: string -> AbstractSyntax.process -> area
  val write: string -> AbstractSyntax.process -> regular_value array
  val read: string -> AbstractSyntax.process -> regular_value array
  val quantitative_semaphore_levelwise: string -> int -> int -> AbstractSyntax.process -> area
...
let switch_semaphore sem il =
  ...
  AC.normalize (aux RV.((*step*) start) (Array.to_list il.code) (AC.empty ~d:1 ()))
    ...
    (** Dans le processus séquentiel *il*, on recherche le lieu des
	points où le processus détient n occurrences du sémaphore *sem*
	d'arité *arity_sem* (c'est donc une région de dimension 1). Lève une
	erreur dans le cas où le processus tente de libérer une occurrence
	de *sem* alors qu'il n'en détient aucune.*)

    (* Plus précisément, la fonction renvoie le lieu des points où le
       processus détient au moins n occurrences du sémaphore, avec un
       phénomène de blocage au cas où il prenne toutes les
       occurrences. En particulier on obtient une chaîne décroissante
       de région en faisant croître le paramètre n. *)
