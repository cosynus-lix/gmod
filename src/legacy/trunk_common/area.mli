...
sig
 ...
 (** Graded boolean algebra of areas. Each area has a rank, that is to say a 
 natural number which is to be thought of as its "geometric dimension". Given 
 two areas x and y of rank d and e the rank of the product x*y is d+e. The 
 collection of areas of rank d forms a boolean algebra A_n. The boolean algebra 
 of areas of null rank, that is to say A_0, is isomorphic to {true,false}. The 
 areas of rank 1 generate the graded boolean algebra in the sense that any 
 element of A_n is a finite join of n-fold products i_1*...*i_n where the rank 
 of each i_k is 1. Such products are called bricks.

  The graded boolean algebras is generated by the areas of rank 1 the way that 
  monomials generate the graded algebra of polynomials.
  
  The standard example is provided by the finite union of hypercubes where * is 
  the Cartesian product, the elements of rank 1 are the intervals of the real 
  line.
  
  A graded boolean algebra might not be freely generated, indeed in the 
  preceding example one has [0,2]U[2,3]=[0,1]U[1,3]. Therefore, since the areas 
  are implemented as finite sets of bricks, we need a normal form to decide 
  whether two values represent the same area. The normal form is actually 
  obtained by calling the complement function two times which returns the 
  collection of maximal sub-bricks. The theoretic explanation of this approach 
  is the Galois connection between finite sets of bricks and areas.
  
  Practically, the product, meet and complement operators preserve the normal 
  forms but the join one does not. *)
