...
struct
  ...
  module CubeMake = Cube.Make(Common)
  module C = CubeMake(I)
  module TorusMake = Torus.Make(Common)
  module T = TorusMake(A)
(*
  module AreaMake = Area.Make(Common)
*)
  module AOCMake = AreaOverCube.Make(Common)
  module AOTMake = AreaOverTorus.Make(Common)
  module AC = AOCMake(C)
  module AT = AOTMake(T)
  ...
  let test n = 
    if n < 0 || 31 < n
    then failwith "Test.ml: test: n has to be taken between 0 and 31"
    else Array.get all_tests n


  let test_Opn_only n = 
    if n < 0 || 31 < n
    then failwith "Test.ml: test_Opn_only: n has to be taken between 0 and 31"
    else Array.get all_tests_Opn_only n


  let test_Cls_only n = 
    if n < 0 || 31 < n
    then failwith "Test.ml: test_Cls_only: n has to be taken between 0 and 31"
    else Array.get all_tests_Cls_only n

  let test_Iso_only n = 
    if n < 0 || 31 < n
    then failwith "Test.ml: test_Iso_only: n has to be taken between 0 and 31"
    else Array.get all_tests_Iso_only n

  let test_Pun_only n = 
    if n < 0 || 31 < n
    then failwith "Test.ml: test_Pun_only: n has to be taken between 0 and 31"
    else Array.get all_tests_Pun_only n
...
(* convert HL.t to AC.t *)
  ...
  | b::a' -> (*The first value should not be Pun x so it has to be treated separately*)
    let b' = match b with (*The first value is stored in order to force osff updating*)
      ...
      | Sh.Iso x -> Sh.Opn x
      | _ -> failwith "Invalid form detected [Test.next_oda]" in
    let answer =
      b'::List.map 
        (
    fun b -> 
  ...
  | _ -> raise Exit
    in
    try next_oda a (*First exhaust all combination of bound types...(1)*)
    with Exit -> (*(1)...then exhaust all possible integer values...(2)*)
  (
    try
      let a = (*Globals.*)Algebra.SequencesOfIntegers.next_list (List.map Sh.rvb a) upper_bound in
    ...
    then raise Exit (*Can't go any further*)
      ...
      List.map 
        (fun x -> Sh.Opn x) 
        (Common.List.initial_segment next_level)
        )
  )
  ...
  let show_them_all_up_to ?upper_bound len =
    try
      ...
      let rec show_them_all a = 
        ...
        show_them_all (next_oda ~upper_bound:upb ~max_length:len a) in
      ...
      then
        (
          let () = Printf.printf
            ...
            (AC.string_of oracle_return) in
          raise Exit
        )
    with Failure "binary_boolean_operator NIY [Oda]" -> incr niy_counter
...
"
  ...
  with 
    ...
    try go [] []
    with 
      | Exit -> print_endline "All tests were successfully passed."
...
let run_tests p = 
  let mismatch_accumulator = ref false in
  let set_mismatch () = mismatch_accumulator := true in
  let update_mismatch () = mismatch_accumulator := !mismatch_accumulator || not !Sheet_solver_dgrog.agreement in
  match p with
    | Command.Oda -> ()
    | Command.Dgrog -> print_endline "Testing the Dgrog library" ;
      (
  let test_files = List.filter 
    (fun filename -> Filename.check_suffix filename !Parameter.Suffix.dgrog) 
    (Array.to_list (Sys.readdir !Parameter.Directory.source_tests)) 
  in
  if test_files = []
  then
    Printf.printf "%s no test found in directory %S\n" 
      Message.warning 
      !Parameter.Directory.source_tests
  else
    List.iter
      (
        fun filename -> 
    Printf.printf " - check file %S: " filename;
    try
      let return = Sheet_solver_dgrog.from_filename (!Parameter.Directory.source_tests^filename)     
      in
      let return = Expression.map 
        (
          fun mr -> match mr with 
      | Sheet_solver_dgrog.Dgrog g -> g 
      | _ -> failwith "Crash"
        ) return
      in
      let expected = Sheet_solver_dgrog.from_filename 
        (!Parameter.Directory.expected_results^filename) in
      ...
      in
      Sheet_solver_dgrog.confront return expected ;
      print_endline (if !Sheet_solver_dgrog.agreement then "done" else " ");
      update_mismatch () ;
    with
      | Failure s -> set_mismatch () ; print_endline s
      | _ -> set_mismatch ();
        Printf.printf "%s no expected results file found for test %s\n" 
          Common.Terminal.(color Red ~bold:true "Error:") 
          Common.Terminal.(color Red filename)
      )
      test_files
  ;
  if !mismatch_accumulator
  then print_endline Common.Terminal.(color Red ~bold:true "The Dgrog library and the Oracle disagree")
  else print_endline Common.Terminal.(color Blue ~bold:true "All tests were successfully passed")
      )
    | _ -> ()
